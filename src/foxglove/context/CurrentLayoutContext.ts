...
import { VariableValue } from "../types/studio";
import { PanelConfig, PlaybackConfig, UserNodes } from "../types/panels";
...
import toggleSelectedPanel from "../panel/toggleSelectedPanel";
...
export interface ICurrentLayout {
  ...
  addSelectedPanelIdsListener: (
    listener: (_: readonly string[]) => void
  ) => void;
  ...
  actions: {
    ...
    updatePanelConfigs: (
      panelType: string,
      updater: (config: PanelConfig) => PanelConfig
    ) => void;
...
const useShouldNotChangeOften = <T>(value: T, warn: () => void): T => {
...
const selectWithUnstableIdentityWarning = <T, U>(
  value: T,
  selector: (value: T) => U
): U => {
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    if (result !== secondResult) {
      console.log(
        `Selector ${selector.toString()} produced different values for the same input. This will cause unecesessery re-renders of your component.`
      );
    }
    return secondResult;
  }
...
const CurrentLayoutContext = createContext<ICurrentLayout | undefined>(
  undefined
);
...
export function useCurrentLayoutSelector<T>(
  selector: (layoutState: LayoutState) => T
): T {
  console.log(CurrentLayoutContext);
  ...
  const state = useRef<{ value: T; selector: typeof selector } | undefined>(
    undefined
  );
...
export function useSelectedPanels(): SelectedPanelActions {
  ...
  const setSelectedPanelIds =
    useGuaranteedContext(CurrentLayoutContext).setSelectedPanelIds;
  const getSelectedPanelIds =
    useGuaranteedContext(CurrentLayoutContext).getSelectedPanelIds;
  const { getCurrentLayoutState: getCurrentLayout } = useCurrentLayoutActions();

  const selectAllPanels = useCallback(() => {
    // eslint-disable-next-line no-restricted-syntax
    const panelIds = getLeaves(
      getCurrentLayout().selectedLayout?.data?.layout ?? null
    );
