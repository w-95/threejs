...
import { AxisMark, MARK_TYPE } from "./AxisMark"; //yxd lastpoint
...
const DEFAULT_LINE_WIDTH = 0.06;  //修改线默认的宽带，原默认值为0.2
const DEFAULT_GRADIENT: GradientRgba = [
  // { r: 124 / 255, g: 107 / 255, b: 1, a: 1 },
  // { r: 124 / 255, g: 107 / 255, b: 1, a: 0.5 },
  { r: 75/255, g: 250/255, b: 250/255, a: 1 },  //修改路线默认的颜色  yxd 20230725
  { r: 75/255, g: 250/255, b: 250/255, a: 1 },  //修改路线默认的颜色  yxd 20230725
];
...
export type PoseArrayUserData = BaseUserData & {
  ...
  axes: Axis[];
  lastPointAxes:AxisMark[];//yzd lastpoint
...
export class PoseArrays extends SceneExtension<PoseArrayRenderable> {
  ...
  //geometry_msgs/PoseArray这种主题数据接收到时触发
  ...
  //foxglove.PosesInFrame这种主题数据接收到时触发
  ...
  // yxd lastpoint
  private _createAxesToMatchPosesforLastPoint(
    renderable: PoseArrayRenderable,
    // poseArray: PoseArray,
    topic: string,
  ): void {
    const scale = renderable.userData.settings.axisScale * (1 / AXIS_LENGTH);
    if(renderable.userData.lastPointAxes.length>0){
      const axis = renderable.userData.lastPointAxes[0]!;
      axis.visible = true;
      axis.scale.set(scale, scale, scale);
    }
    else{
      // const axis = new Axis(topic, this.renderer);
      const axisLastPoint = new AxisMark(topic, this.renderer,MARK_TYPE.END_MARK,true);
      renderable.userData.lastPointAxes.push(axisLastPoint);
      renderable.add(axisLastPoint);
      axisLastPoint.scale.set(scale, scale, scale);
    }
  }
  ...
  private _updatePoseArrayRenderable(
    ...
    switch (settings.type) {
      ...
      case "line": {
        ...
        renderable.userData.lineStrip?.update(lineStripMarker, undefined);
        //yxd lastpoint
        this._createAxesToMatchPosesforLastPoint(renderable, topic);
        let tempi=poseArrayMessage.poses.length-1;
        setObjectPoseForLastPoint(renderable.userData.lastPointAxes[0]!, poseArrayMessage.poses[tempi]!);
        break;
      }
    }
  }
...
//yxd lastpoint
function setObjectPoseForLastPoint(object: THREE.Object3D, pose: Pose): void {
  const p = pose.position;
  const q = pose.orientation;
