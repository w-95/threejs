...
export class FrameAxes extends SceneExtension<FrameAxisRenderable> {
  ...
  public constructor(renderer: IRenderer) {
    ...
    //构造函数中调用handleTransformTreeUpdated，更新transform tree
  ...
  public override settingsNodes(): SettingsTreeEntry[] {
    ...
    //3D设置面板上“Transforms/变换” 节点的“Frame”构建，根据数据通过循环构建  yxd 20230521
    let order = 1;
  ...
  //3D设置面板上“Transforms/变换” 节点的“设置”选项中修改设置后的处理事件  yxd 20230521
  ...
  private handleTransformTreeUpdated = (): void => {
    for (const frameId of this.renderer.transformTree.frames().keys()) {
      ...
      // {
      //   this._addFrameAxis(frameId);//更换为其它图标
      // }      
  ...
  private _addFrameAxis(frameId: string): void {
    ...
    renderable.add(axis);  //在3D面板上添加transform的坐标箭头 yxd5
    renderable.add(label); //在3D面板上添加transform的标签名称 yxd5
    renderable.add(parentLine);  //在3D面板上添加transform的坐标箭头到标签上的黄色线 yxd5
