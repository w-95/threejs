...
export class Urdfs extends SceneExtension<UrdfRenderable> {
  ...
  public constructor(renderer: IRenderer) {
    ...
    // 如果添加了自定义图层，就会根据config文件循环处理自定义图层，这里仅处理URDF自定义图层，但还是要循环遍历所有自定义图层
  ...
  private _fetchUrdf(instanceId: string, url: string): void {
    ...
    //zwd3：proxy反向代理
    //url='https://x.droid.ac.cn/devtest/zkzdx_v8.stl';
    //变为：
    //url='droid/devtest/zkzdx_v8.stl';
    // url=url.replace('https://x.droid.ac.cn/','droid/');
    //console.log('zwd3:',url);  
...
function createRenderable(
  ...
  //yxd 解决URDF模型悬空的问题，方法1：从代码中减去Z方向的偏移，这个偏移值是否固定。2023-06-26
  let tempposition:Vector3=visual.origin.xyz;
  tempposition.z=tempposition.z-0.18;
  const pose = { position: tempposition, orientation };
  // const pose = { position: visual.origin.xyz, orientation };// yxd 原来的姿态设置方式
