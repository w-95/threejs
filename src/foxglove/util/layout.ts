...
import { TabConfig, TabLocation, TabPanelConfig } from "../types/layouts";
import {
  PanelConfig,
...
export function isTabPanel(panelId: string): boolean {
  return getPanelTypeFromId(panelId) === 'Tab';
}
...
export const getSaveConfigsPayloadForAddedPanel = ({
  ...
  const newConfigs = filterMap(templateIds, (templateId: any) => {
    ...
    if (tabProps?.tabs) {
      tabProps.tabs.forEach((tab: TabConfig) => {
        tabLayouts.push(
          ...
          ...getPanelIdsInsideTabPanels(getLeaves(tab.layout ?? null), savedProps),
        );
      });
    }
  });
